-- =============================================================
-- 효과카드 뽑기 비용 점증 방식 변경: 덧셈(200 + 50*n) → 2배(200 * 2^n).
-- 0001~0033(또는 0032 까지) 실행 후 이 파일을 실행하세요.
-- (draw_effect_card 만 재정의하므로 0033 뱅크롤과 독립적으로 적용 가능.)
--
--  · 비용식: v_cost := 200 * 2^v_paid  (뽑을수록 2배씩 점증)
--    유료 1번째 200 · 2번째 400 · 3번째 800 · 4번째 1,600 · 5번째 3,200 …
--    ⚠ lib/constants.ts GACHA_BASE(200) * GACHA_MULT(2)^paid_count 와 값 일치
--  · 카드 남발로 인한 인플레이션 억제 목적(뽑을수록 기하급수적으로 비싸짐).
--  · 나머지 로직(무료 3연차·등급확률·중복환급 50%)은 0016/0027 과 동일.
-- =============================================================
...
begin
  ...
  if v_isfree then
    ...
    -- 2배 점증. 상한을 둬 int 오버플로 방지(현실적으로 도달 불가한 값).
    -- ⚠ lib/constants.ts GACHA_BASE(200) * GACHA_MULT(2)^paid_count 와 값 일치
    v_cost := least(200 * power(2::numeric, v_paid), 2000000000)::int;
