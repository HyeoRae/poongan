-- =============================================================
-- 가챠(효과카드 뽑기) 비용 상향: 30/+15 → 200/+50.
-- 0016 의 draw_effect_card 를 그대로 두고 비용식 한 줄만 바꿔 재정의한다.
-- (효과카드 영향력이 커서 뽑기 비용을 비싸게 조정)
-- 0001~0026 실행 후 이 파일을 실행하세요.
--
--  · 비용식: v_cost := 200 + 50 * v_paid  ⚠ lib/constants.ts GACHA_BASE(200)/GACHA_STEP(50) 와 값 일치
--  · 나머지 로직(무료 3연차·등급확률·중복환급 50%)은 0016 과 동일.
-- =============================================================
...
begin
  ...
  if v_isfree then
    ...
    v_cost := 200 + 50 * v_paid;                      -- 비용 점증. ⚠ lib/constants.ts GACHA_BASE(200)/GACHA_STEP(50) 와 값 일치
